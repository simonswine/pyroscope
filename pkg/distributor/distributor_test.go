...
import (
	...
	pprofhelper "github.com/grafana/pyroscope/pkg/pprof/testhelper"
...
func TestPush_LabelRewrites(t *testing.T) {
	...
	for idx, tc := range []struct {
	...
	}{
		{
			name:           "empty series",
			series:         []*typesv1.LabelPair{},
			expectedSeries: `{__name__="process_cpu", service_name="unspecified"}`,
		},
		{
			name: "series with . labels",
			series: []*typesv1.LabelPair{
				{Name: "service.name", Value: "my-service"},
				{Name: "cloud.region", Value: "my-region"},
			},
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			p := pprofhelper.NewProfileBuilderWithLabels(1000*int64(idx), tc.series).CPUProfile()

...
func (m *InvokeOptions) CloneVT() *InvokeOptions {
	...
	r.UseAttributeIndex = m.UseAttributeIndex
	if len(m.unknownFields) > 0 {
		r.unknownFields = make([]byte, len(m.unknownFields))
		copy(r.unknownFields, m.unknownFields)
	}
...
func (this *InvokeOptions) EqualVT(that *InvokeOptions) bool {
	...
	if this.UseAttributeIndex != that.UseAttributeIndex {
		return false
	}
	return string(this.unknownFields) == string(that.unknownFields)
}
...
func (m *InvokeOptions) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 3:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field UseAttributeIndex", wireType)
			}
			var v int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
